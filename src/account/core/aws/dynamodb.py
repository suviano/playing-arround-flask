...
class DynamoResource:
    ...
    def error_help_strings(cls, error_code):
        # Got this map from the NoSQL Workbench from aws (Shame it's not in
        # already in the library)
        # https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/workbench.html
        return {
            "InternalServerError": (
                "Internal Server Error, generally safe to "
                "retry with exponential back-off"
            ),
            "ProvisionedThroughputExceededException": (
                "Request rate is too high. If you're using a custom retry "
                "strategy make sure to retry with exponential back-off."
                "Otherwise consider reducing frequency of requests or"
                " increasing provisioned capacity for your table or"
                " secondary index"
            ),
            ...
            "ServiceUnavailable": (
                "Had trouble reaching DynamoDB. generally"
                " safe to retry with exponential back-off"
            ),
            "ThrottlingException": (
                "Request denied due to throttling, generally safe to retry "
                "with exponential back-off"
            ),
            "UnrecognizedClientException": (
                "The request signature is incorrect most likely due to "
                "an invalid AWS access key ID or secret key,"
                " fix before retrying"
            ),
            "ValidationException": (
                "The input fails to satisfy the constraints specified by"
                " DynamoDB, fix input before retrying"
            ),
            "RequestLimitExceeded": (
                "Throughput exceeds the current throughput limit for "
                "your account, increase account level throughput "
                "before retrying"
            ),
        }[error_code]
